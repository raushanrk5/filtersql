...
func TestMySQL_ScalarAndLike(t *testing.T) {
	db := mySetup(t)
	where, args, _ := myReg.Compile(dialects.MySQL{}, []fq.Condition{
...
func TestMySQL_JSONArrayAndMap(t *testing.T) {
	...
	w1, a1, _ := myReg.Compile(dialects.MySQL{}, []fq.Condition{{Key: "tags", Op: fq.OpContainsAny, Values: []any{"api", "db"}}})
	...
	w2, a2, _ := myReg.Compile(dialects.MySQL{}, []fq.Condition{{Key: "tags", Op: fq.OpContains, Values: []any{"prod", "crit"}}})
	...
	w3, a3, _ := myReg.Compile(dialects.MySQL{}, []fq.Condition{
...
func TestMySQL_NullKeysetAndNullsEmulation(t *testing.T) {
	...
	// NULL owner -> a2
	wn, an, _ := myReg.Compile(dialects.MySQL{}, []fq.Condition{{Key: "owner", Op: fq.OpIsNull}})
	...
	b := myReg.Builder(dialects.MySQL{})
...
func TestMySQL_Having(t *testing.T) {
	db := mySetup(t)
	_, having, args, _ := myReg.CompileWhereHaving(dialects.MySQL{}, nil,
